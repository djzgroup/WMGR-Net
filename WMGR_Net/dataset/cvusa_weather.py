...
import time
from WMGR_Net.transforms_weather_label import WeatherDegradation
...
class CVUSADatasetTrain(Dataset):
    ...
    def __init__(self,
        ...
        self.df = pd.read_csv(f'{data_folder}/splits/splits/train-19zl.csv', header=None)
    ...
    def _get_weather_aug(self, weather_id):
        if weather_id == 1:  # Fog
            return WeatherDegradation(intensity=0.6, weather_type='fog', p=1.0)
        elif weather_id == 2:  # Rain
            return WeatherDegradation(intensity=0.8, weather_type='rain', p=1.0)
        elif weather_id == 3:  # Snow
            return WeatherDegradation(intensity=0.7, weather_type='snow', p=1.0)
        return None
    ...
    def __getitem__(self, index):
        ...
        street = ground.split('/')[0]
        query_img = cv2.imread(f'{self.data_folder}/{street}/{ground}')
        # query_img = cv2.imread('F:\locatication数据集\CVPR_subset\streetview\panos\\0036029.jpg')
        query_img = cv2.cvtColor(query_img, cv2.COLOR_BGR2RGB)

        # load reference -> satellite image
        binmap = sat.split('/')[0]
        reference_img = cv2.imread(f'{self.data_folder}/{binmap}/{sat}')
        ...
        # 训练阶段，50%概率触发 EDA (仅对地面视角 query)
        # 注意：这里我们手动应用天气，然后再过 transforms_query (通常包含 Normalize)
        if np.random.random() < 0.5:
            # 随机选择一种天气 (1: Fog, 2: Rain, 3: Snow)
            w_id = np.random.randint(1, 4)
            aug = self._get_weather_aug(w_id)
            if aug:
                # albumentations 风格调用
                query_img = aug(image=query_img)['image']
                weather_label = w_id
        # ----------------- 核心修改结束 -----------------
        ...
        return query_img, reference_img, label, torch.tensor(weather_label, dtype=torch.long)
...
class CVUSADatasetEval(Dataset):
    ...
    def __init__(self,
        ...
        if split == 'train':
            ...
            self.df = pd.read_csv(f'{data_folder}/splits/splits/val-19zl.csv', header=None)
    ...
    def __getitem__(self, index):
        binmap = self.images[index].split('/')[0]
        img = cv2.imread(f'{self.data_folder}/{binmap}/{self.images[index]}')
