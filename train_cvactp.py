...
from WMGR_Net.dataset.cvact_weather import CVACTDatasetTrain, CVACTDatasetEval, CVACTDatasetTest
from WMGR_Net.transforms_weather_label import get_transforms_train, get_transforms_val
from WMGR_Net.utils import setup_system, Logger
from WMGR_Net.trainer_weather_label import train
from WMGR_Net.evaluate.cvusa_and_cvact import evaluate, calc_sim
from WMGR_Net.model_wmgr import WMGR_Net
from WMGR_Net.model_wmgr import WMGRLoss


@dataclass
class Configuration:
    # Model
    model: str = 'convnextv2_base.fcmae_ft_in22k_in1k_384'
    ...
    neighbour_range: int = 32  # pool size for selection
    gps_dict_path: str = "/smalldata/CVACT/gps_dict.pkl"  # path to pre-computed distances
    ...
    # Learning Rate
    lr: float = 0.00008  # 1 * 10^-4 for ViT | 1 * 10^-1 for CNN
    scheduler: str = "cosine"  # "polynomial" | "cosine" | "constant" | None
    warmup_epochs: int = 1
    lr_end: float = 0.0001  # only for "polynomial"

    # Dataset
    data_folder = "/smalldata/CVACT"
...
if __name__ == '__main__':
    ...
    # 替换原来的InfoNCE
    loss_function = WMGRLoss(
        loss_function=loss_fn,
        device=config.device,
        local_weight=0.2,  # 可以从0.2开始调试
        lc_weight=0.1  # 可以从0.1开始调试
    )
    weather_loss_function = torch.nn.CrossEntropyLoss(label_smoothing=config.label_smoothing).to(config.device)
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        train_loss = train(config,
                           ...
                           weather_loss_function=weather_loss_function,
                           optimizer=optimizer,
                           scheduler=scheduler,
                           scaler=scaler)
